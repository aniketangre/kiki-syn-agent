...
agent3.py
---------
Identical to agent2.py with one difference: error resilience via LLM fallback.

Difference from agent2.py
--------------------------
LLM setup uses OpenAI (gpt-4o-mini) as the primary model with Google Gemini
(gemini-2.5-flash) as an automatic fallback. If OpenAI raises a rate limit,
connection, timeout, or server error, LangChain silently retries the same call
with Gemini — no code change required in the rest of the file.

The fallback is applied separately to:
  _base_llm       — used by KIKI agent, Synera agent, and responder node
  _supervisor_llm — uses with_structured_output(), so each model gets it
                    applied individually before the fallback chain is built

Errors that trigger the fallback (OpenAI infrastructure failures):
  openai.RateLimitError
  openai.APIConnectionError
  openai.APITimeoutError
  openai.InternalServerError

Errors that do NOT trigger the fallback (must be fixed by the operator):
  openai.AuthenticationError  — wrong API key, human fix required

To use:
    Change the import in app.py from agent2 to agent3.
    Both OPENAI_API_KEY and GOOGLE_API_KEY must be set in .env.
"""
...
# Errors that indicate an OpenAI infrastructure failure — fall back to Gemini.
# AuthenticationError is intentionally excluded: a bad key needs a human fix,
# not a silent retry on a different model.
_FALLBACK_ON = (
    openai.RateLimitError,
    openai.APIConnectionError,
    openai.APITimeoutError,
...
# Base LLM used by sub-agents and the responder node
_base_llm = _primary_llm.with_fallbacks(
    [_fallback_llm],
    exceptions_to_handle=_FALLBACK_ON,
)

# Supervisor LLM — structured output applied per model before chaining,
# because with_structured_output() must wrap each model individually.
...
# ---------------------------------------------------------------------------
# Terminal interactive loop (python agent3.py)
# ---------------------------------------------------------------------------

if __name__ == "__main__":

    print("=" * 60)
    print("  Synera AI Agent — Supervisor Multi-Agent + LLM Fallback")
